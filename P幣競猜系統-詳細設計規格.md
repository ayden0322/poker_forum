...
### 實作狀態（2026-07-07 對帳，分支 feat/p-coin-prediction）
✅ 全部完成並本地驗證：schema 三件套、賠率管線、下注 API、結算、對帳+admin 沖正、公開盤口 API、
前端（競猜中心/BetSlip/排行榜/戰績頁/G→P 兌換）、結算通知。100 單測 + next build + 瀏覽器真實路徑
（下注 8/8、兌換榜戰績 5/5）。三輪 Codex 複審（下注 6 項、結算 3 項）全採納修畢。
**go-live checklist**：①prod 跑 `prisma/prod-hardening-predictions.sql` ②Zeabur 設 `PREDICTION_ENABLED=true`
③確認 `MEMBER_ECONOMY_ENABLED=true`（兌換依賴）④merge 後首日盯 odds 額度 counter 與對帳 log。
...
- [ ] **prod 加固（Codex 複審 H3，go-live 前補跑）**：OddsQuote 加 partial unique index 擋跨 instance 併發寫盤——`CREATE UNIQUE INDEX CONCURRENTLY odds_quotes_active_combo ON odds_quotes (match_id, bookmaker_id, market, selection, COALESCE(line, -1)) WHERE active = true;`（Prisma schema 表達不了 partial unique，手寫 SQL；已實測 db push 不會抹掉，但新環境要記得補。單 instance 期間由行程內互斥 + updateMany 全量翻舊自癒頂著）

### Codex 複審紀錄（2026-07-07，結算）
高×2 中×1 全數採納修復：H1 改日/凍結賽事會消失在 startTime<=now 掃描外（退款永不觸發）且關場賽事可被
API 翻回 NS 復活收注 → 掃描加 OR 條件+改日重開不蓋章+收單/寫盤閘門加 settledAt/frozenAt；H2 AET/PEN 的
goals 是含延長賽總比分、fallback 會錯結 90 分鐘盤 → 只認 score.fulltime、goals 限定 FT；M 結算用舊
snapshot 可能蓋掉同步剛更新的比分 → per-board 互斥+結算前重讀。查過沒問題：退款/派彩互斥（狀態機承重）、
ids 批次、markMatchSettled 蓋章條件、creditInTx balanceAfter、decideOutcome。
驗證：6 場景生命週期（贏/輸/平、重跑冪等、凍結期滿退款、未知狀態、改日重開接新注、關場復活防禦）全綠。

### Codex 複審紀錄（2026-07-07，下注 API）
高×3 中×2 低×1 全數採納修復：H1 限額聚合移進交易+錢包行鎖串行化、H2 交易內重查 quote、H3 寫盤互斥+updateMany 自癒（partial unique 留 prod 加固）、M1 派彩改 Decimal 計算（100×1.15 浮點會少 1P）、M2 clientRequestId 請求級冪等、L1 Redis 計數改原子 INCR。查過沒問題：debitInTx 原子性、交易順序、single-flight 清理、quoteId 換用繞過、DTO 驗證。
