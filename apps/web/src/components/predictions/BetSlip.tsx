...
// P幣競猜 — Bet Slip（兩步制 bottom sheet，設計規格 §7.1-7.3）
// 鐵律：
//   - 點賠率零後果，風險全集中在唯一確認鍵上；確認鍵文案永遠印著鎖定賠率
//   - 賠率變動不准靜默換值：ODDS_CHANGED → 整顆鍵變 acknowledge（按過「繼續」才恢復可送出）
...
export default function BetSlip({
  ...
  const odds = active.quote.odds;
  const payout = stake ? Math.floor(stake * odds) : 0;
  const selLabel =
  ...
  const submit = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['member'] }); // P 幣餘額徽章
    } catch (e) {
      if (e instanceof ApiError) {
        ...
        switch (e.code) {
          ...
          case 'INSUFFICIENT_BALANCE':
            ...
            return;
          case 'LIMIT_EXCEEDED':
            setPhase({ kind: 'notice', message: e.message || '超出投注上限' });
            return;
      ...
      <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
        ...
        >
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl leading-none" aria-label="關閉">
              ×
            </button>
          </div>
              ...
              <div className="mt-4 flex gap-2">
                {CHIPS.map((c) => (
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg border text-sm font-mono-stadium tabular-nums transition-colors ${
                      !customMode && stake === c
