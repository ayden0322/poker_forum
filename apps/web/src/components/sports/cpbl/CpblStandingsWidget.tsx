...
interface StandingTeam {
  position: number;
  team: { id: number; name: string; logo: string; nameZhTw?: string };
  games?: {
    played: number;
...
function flattenStandings(raw: StandingTeam[] | StandingTeam[][] | undefined): StandingTeam[] {
  if (!raw) return [];
...
const TEAM_ZH: Record<number, string> = {
  348: '中信兄弟',
  349: '富邦悍將',
  482: '樂天桃猿',
  569: '味全龍',
  351: '統一獅',
  // 台鋼雄鷹 的 ApiSports id 視回傳結果決定
};

function teamLabel(t: StandingTeam): string {
  return t.team.nameZhTw ?? TEAM_ZH[t.team.id] ?? t.team.name;
}

export function CpblStandingsWidget() {
  const { data, isLoading, isError } = useQuery({
    queryKey: ['cpbl-standings'],
    queryFn: () => apiFetch<StandingsResponse>('/baseball/cpbl/standings'),
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden mb-4">
      <div className="px-3 py-2 border-b border-gray-100 bg-gradient-to-r from-red-50 to-white flex items-center gap-2">
        <span>📊</span>
        <h3 className="font-bold text-gray-800 text-sm">中華職棒戰績排行榜</h3>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-2 py-2">
                      <Link
                        href={`/team/baseball/cpbl/${row.team.id}`}
