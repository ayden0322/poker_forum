...
function TopFiveCard({ onViewFull }: { onViewFull: () => void }) {
  ...
  return (
    <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
      ...
      {rows.length === 0 ? (
        <div className="mt-3 text-xs text-gray-400">滿 {minSettled} 場競猜即可入榜</div>
      ) : (
        <div className="mt-3 space-y-2">
          {rows.map((r) => (
            <div key={r.nickname} className="flex items-center gap-2 text-xs">
              ...
              <span className={`font-mono-stadium tabular-nums font-bold ${r.profit >= 0 ? 'text-[#2a8d92]' : 'text-gray-500'}`}>
