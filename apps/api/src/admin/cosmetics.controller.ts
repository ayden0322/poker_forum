...
class CreateCosmeticDto {
  ...
  @IsOptional() @IsString() @MaxLength(60) iconKey?: string;  // 勳章 lucide 名稱（kebab-case）
  @IsOptional() @IsString() @MaxLength(500) assetUrl?: string; // 圖檔配件 URL（頭像框/勳章圖）
...
class UpdateCosmeticDto {
  ...
  @IsOptional() @IsString() @MaxLength(500) assetUrl?: string;
  @IsOptional() @IsEnum(Rarity) rarity?: Rarity;
...
export class AdminCosmeticsController {
  ...
  async create(@Body() dto: CreateCosmeticDto) {
    this.assertIcon(dto.type, dto.iconKey, dto.assetUrl);
  ...
  /** 勳章需要 lucide iconKey 或 圖檔 assetUrl；頭像框可用 assetUrl 或純 CSS 環(由 rarity 決定) */
  private assertIcon(type: CosmeticType, iconKey?: string, assetUrl?: string) {
    if (type === CosmeticType.BADGE && !iconKey && !assetUrl) {
      throw new BadRequestException('勳章需要指定 lucide 圖示或上傳圖檔');
    }
  }
