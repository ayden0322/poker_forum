...
export class CosmeticsService {
  ...
  async getShop(userId: string, type?: 'FRAME' | 'BADGE' | 'TITLE' | 'EFFECT', now: Date = new Date()) {
    ...
    return {
      ...
      items: items.map((i) => ({
        ...
        iconKey: i.iconKey, assetUrl: i.assetUrl, rarity: i.rarity, priceG: i.priceG, levelRequired: i.levelRequired,
  ...
  async getInventory(userId: string) {
    ...
    return {
      items: rows.map((r) => ({
        itemId: r.itemId, type: r.item.type, name: r.item.name, iconKey: r.item.iconKey, assetUrl: r.item.assetUrl,
